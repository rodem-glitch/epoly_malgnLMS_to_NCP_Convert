...
| GitHub Actions VM SSH 계정 불일치 복구 | `Deploy LMS to GCP + Firebase` 워크플로의 `원클릭 배포 실행 (VM 스택)` 단계 | `tools/gcp/one-click-setup.ps1`, `.github/workflows/deploy-lms-gcp.yml` | SSH 계정 자동 후보 탐색 + one-click 재시도(최대 2회) + 인자 바인딩 오류 제거 | 확인 근거: 최근 실패 run(#11, #12)이 동일하게 VM 스택 단계에서 중단된 이력 확인 후 `Deploy-StackToVm`에 `지정계정→gcloud계정→newkl→ubuntu→기본호스트` 순차 시도 추가, run(#13,#14)에서 재현된 `SourceDbPort` 바인딩 오류를 원인으로 확인해 워크플로 원클릭 호출을 배열 스플랫(`@args`) 방식에서 명시적 파라미터 호출로 복구 |
