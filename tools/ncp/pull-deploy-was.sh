...
# 1-repo 패턴: 앱 소스 + 배포 템플릿이 하나의 저장소에 통합되어 있습니다.
# 사용법: sudo bash pull-deploy-was.sh

# ─── 저장소 설정 ──────────────────────────────────────────────────
# 왜: 1-repo 패턴으로 앱 소스와 배포 스크립트를 단일 저장소에서 관리합니다.
...
# 왜: 단일 저장소를 clone하거나 최신으로 pull합니다.
clone_or_pull() {
  local repo_dir="$1"
  local repo_url="$2"
  local repo_branch="$3"
...
assemble_bundle() {
  ...
  # 왜: 1-repo 패턴이므로 앱 소스와 배포 템플릿 모두 같은 저장소에서 가져옵니다.
  ...
  # JAR 복사
  local jar_file
  jar_file=$(find "${REPO_DIR}/polytech-lms-api/build/libs" -name "*.jar" ! -name "*plain*" | head -1)
  cp "${jar_file}" "${app_dir}/polytech-lms-api.jar"
  log "JAR 복사: ${jar_file}"

  # Docker Compose 복사 (같은 저장소 내 템플릿)
  cp "${REPO_DIR}/tools/ncp/templates/docker-compose-was.yml.tpl" "${BUNDLE_DIR}/docker-compose.yml"

  # 배포 스크립트 복사 (같은 저장소 내 템플릿)
  cp "${REPO_DIR}/tools/ncp/templates/deploy-was.sh.tpl" "${BUNDLE_DIR}/deploy-was.sh"
  chmod +x "${BUNDLE_DIR}/deploy-was.sh"

  # 레거시 소스 복사
  cp -r "${REPO_DIR}/public_html" "${legacy_dir}/public_html"
  cp -r "${REPO_DIR}/src" "${legacy_dir}/src"

  # 통계 데이터 복사
  if [[ -d "${REPO_DIR}/통계" ]]; then
    cp -r "${REPO_DIR}/통계/"* "${stats_dir}/"
    log "통계 데이터 복사 완료."
  fi
  ...
  # resin-web.xml 렌더링 (같은 저장소 내 템플릿)
  sed \
...
main() {
  require_root
  log "=== NCP WAS Pull 배포 시작 (1-repo 패턴) ==="
  install_prerequisites
  clone_or_pull "${REPO_DIR}" "${REPO_URL}" "${REPO_BRANCH}"
  build_jar
  assemble_bundle
  run_deploy
  verify
