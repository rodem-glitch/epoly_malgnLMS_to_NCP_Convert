...
if ($SummaryOnly) {
    Write-Host '[RPG] 모드: SummaryOnly (인덱스(tsv) 재생성 생략)'
}
...
try {
    ...
    $summary = @(
        ...
        '',
        '생성된 인덱스:',
        ('- docs/rpg/generated/jsp_setBody_index.tsv' + $(if ($SummaryOnly) { ' (SummaryOnly에서는 갱신하지 않음)' } else { '' })),
        ('- docs/rpg/generated/jsp_newDao_index.tsv' + $(if ($SummaryOnly) { ' (SummaryOnly에서는 갱신하지 않음)' } else { '' })),
        ('- docs/rpg/generated/dao_table_index.tsv' + $(if ($SummaryOnly) { ' (SummaryOnly에서는 갱신하지 않음)' } else { '' })),
        ('- docs/rpg/generated/polytech_controller_mapping_candidates.tsv' + $(if ($SummaryOnly) { ' (SummaryOnly에서는 갱신하지 않음)' } else { '' }))
    )
    ...
    if (-not $SummaryOnly) {
        ...
        Write-Host ' - (SummaryOnly) 인덱스(tsv) 재생성 생략'
    }
