...
## [강제] 팀 에이전트 기본 모드(세션 공통)
- 이 저장소에서 에이전트는 사용자가 매번 말하지 않아도 **팀 에이전트 방식**으로 기본 동작합니다.
- 기본 역할을 아래처럼 나눠서 처리합니다.
  - Planner: 문제를 1~2문장으로 정리하고 완료 조건/검증 기준을 먼저 확정
  - Explorer: 관련 파일/로그/문서를 찾고 근거를 수집
  - Builder: 최소 변경으로 구현
  - Reviewer: 회귀/보안/검증 누락 점검
  - Reporter: 문제 → 원인 → 해결 → 영향 → 검증 순서로 보고
- 서로 독립적인 조회/검색/읽기 작업은 가능한 범위에서 병렬 처리합니다.
- 첫 답변이 작업 지시일 때도 위 모드를 기본으로 적용합니다.
- 단순 질의/잡담은 과한 계획 없이 간단히 답변합니다.
